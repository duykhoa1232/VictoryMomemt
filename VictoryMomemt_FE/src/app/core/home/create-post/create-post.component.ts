...
export class CreatePostComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.postForm = new FormGroup({
      ...
      allowedUserIds: new FormControl([]), // Mặc định là mảng rỗng, không bắt buộc
    });
    ...
    this.postForm.get('privacy')?.valueChanges.subscribe(privacy => {
      ...
      if (privacy === 'PRIVATE') {
        ...
        // allowedUserIdsControl?.setValidators(Validators.required);
        // allowedUserIdsControl?.markAsUntouched();
        // allowedUserIdsControl?.markAsPristine();
      } else {
        allowedUserIdsControl?.clearValidators(); // Xóa validator nếu có
  ...
  onSubmit(): void {
    ...
    // Gửi allowedUserIds CHỈ KHI privacy là PRIVATE VÀ CÓ NGƯỜI DÙNG ĐƯỢC CHỌN
    // Không bắt buộc phải có người dùng khi PRIVATE
    if (postData.privacy === 'PRIVATE') {
      const allowedUserIds = this.selectedAllowedUsers.map(user => user.id);
      // Backend của bạn mong đợi một mảng JSON stringify hoặc các append riêng lẻ.
      // Nếu BE mong đợi @RequestParam List<String> allowedUserIds, thì bạn cần append từng ID một.
      // Nếu BE mong đợi một JSON string, thì cách này là đúng.
      // Tôi giả định BE mong đợi mảng String được truyền qua JSON string.
      // Nếu có vấn đề, bạn cần điều chỉnh cách BE nhận hoặc FE gửi.
      // Chỉ append nếu có người dùng được chọn
      if (allowedUserIds.length > 0) {
        formData.append('allowedUserIds', JSON.stringify(allowedUserIds));
      }
      // Nếu allowedUserIds rỗng, không append gì cả, backend sẽ hiểu là private cho chính mình hoặc không có người dùng nào khác
    } else {
      // Đảm bảo không gửi allowedUserIds khi không phải PRIVATE
      formData.delete('allowedUserIds');
    }
