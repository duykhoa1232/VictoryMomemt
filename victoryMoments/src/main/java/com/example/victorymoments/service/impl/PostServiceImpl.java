...
//                .privacy(request.getPrivacy())
    ...
    public PostResponse updatePost(String id, PostRequest request) {
        ...
        if (request.getImages() != null && !request.getImages().isEmpty()) {
            ...
            // Upload các file ảnh mới và cập nhật URLs
            ...
            // Nếu danh sách ảnh được truyền lên là rỗng (người dùng muốn xóa tất cả ảnh)
            deleteOldFilesFromS3(existingPost.getImageUrls());
            existingPost.setImageUrls(new ArrayList<>()); // Đặt lại danh sách ảnh rỗng
        }
        // Nếu request.getImages() là null, giữ nguyên ảnh cũ (không thay đổi)


        // Xử lý Video URLs (tương tự)
