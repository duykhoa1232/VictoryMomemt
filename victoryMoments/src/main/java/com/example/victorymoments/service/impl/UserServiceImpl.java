...
public class UserServiceImpl implements UserService {
    ...
    public ProfileResponse getUserProfileByEmail(String emailToFetch, Pageable pageable) {
        User user = userRepository.findByEmail(emailToFetch)
                .orElseThrow(() -> new RuntimeException("User not found with email: " + emailToFetch));
        Page<Post> userPostsPage = postRepository.findByUserIdAndIsActiveTrue(user.getId(), pageable);
        Page<PostResponse> userPostResponsesPage = userPostsPage.map(this::mapToPostResponse);
        return mapToProfileResponse(user, userPostResponsesPage);
    }
    ...
    public ProfileResponse updateUserProfile(String userEmail, ProfileRequest request) {
        ...
        Page<PostResponse> emptyPostsPage = new PageImpl<>(List.of(), Pageable.unpaged(), 0);

        return mapToProfileResponse(updatedUser, emptyPostsPage);
    }
    ...
    private String extractS3KeyFromUrl(String s3Url) {
        try {
            ...
            // Log the parsing error but continue to throw standard exception
