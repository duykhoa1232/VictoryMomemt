...
public class S3Service {
    ...
    public void deleteFile(String key) {
        try {
            ...
            logger.info("Deleted file in S3: {}", key);
        } catch (S3Exception e) {
            logger.error("Error when deleting file on S3", e);
            throw new S3UploadException("Error when deleting file on S3", e);
        }
    }
    ...
    public String generatePresignedUrl(String key, int expiryInMinutes) {
        try {
            ...
            logger.error("Error creating temporary URL", e);
            throw new RuntimeException("Error creating temporary URL", e);
        }
    }
    ...
    private void validateFileSize(MultipartFile file) {
        if (file.getSize() > maxUploadSize) {
            throw new IllegalArgumentException("File exceeds maximum allowed size: " + file.getSize());
        }
    }
    public List<String> listAllKeys(String prefix) {
        ...
        try {
            ...
            do {
                ...
                listResponse.contents().forEach(s3Object -> keys.add(s3Object.key()));
                listRequest = listRequest.toBuilder().continuationToken(listResponse.nextContinuationToken()).build();
            } while (listResponse.isTruncated());
        } catch (S3Exception e) {
            logger.error("Error listing S3 keys: ", e);
            throw new S3UploadException("Error listing S3 keys", e);
        }
        return keys;
    }
