...
public class PostServiceImpl implements PostService {
    ...
    public PostResponse createPost(PostRequest request) {
        ...
        if (visibilityStatus == VisibilityStatus.PRIVATE) {
            authorizedViewerIds.add(userId); // người tạo
        ...
        // Upload media và liên kết postId
        ...
        postRepository.save(savedPost); // lưu lại danh sách mediaIds
    ...
    public Page<PostResponse> getPostsForCurrentUser(Pageable pageable, String userId) {
        ...
        List<Post> combined = Stream.concat(ownedPosts.getContent().stream(), sharedPosts.getContent().stream())
                .distinct().sorted(Comparator.comparing(Post::getCreatedAt).reversed()).collect(Collectors.toList());
    ...
    public Page<PostResponse> getPostsByEmail(String userEmail, Pageable pageable, String currentUserId) {
        ...
        return getPostsByUserId(targetUser.getId(), pageable, currentUserId);
    }
    ...
    public PostResponse updatePost(String id, PostRequest request, String deletedMediaUrls, String userId) {
        ...
        if (existingPost.getVisibilityStatus() == VisibilityStatus.PRIVATE) {
            ...
            authIds.add(userId);
            if (request.getSharedWithUserIds() != null) authIds.addAll(request.getSharedWithUserIds());
            existingPost.setAuthorizedViewerIds(authIds);
        } else {
    ...
    public void deletePost(String id, String userId) {
        ...
        // Xóa toàn bộ media của post
        List<Media> medias = mediaService.listByPostId(post.getId());
        medias.forEach(m -> mediaService.deleteMedia(m.getId()));
